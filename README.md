...
## 🕷️ نظام جلب وتحديث الفتاوى الدوري (Fatwa Scraper)

تم دمج نظام جلب الفتاوى (Scraper) كأداة إدارية مدمجة ضمن مشروع Django. يقوم النظام بجلب الفتاوى الجديدة تلقائياً من موقع دار الإفتاء الرسمي (`ifta.ly`) وحفظها في قاعدة بيانات Supabase بعد توليد المتجهات الدلالية (Embeddings) لها.

### مميزات النظام:
1. **الفحص والتحديث الدوري (Periodic Scan)**: إمكانية تشغيل الفاحص بشكل دوري كعملية خلفية (Daemon) أو تشغيله تلقائياً مع خادم Django.
2. **التحديث التدريجي الذكي (Incremental Scraping)**: يفحص النظام الفتاوى بدءاً من الأحدث. بمجرد العثور على فتاوى تم إدخالها مسبقاً في قاعدة البيانات بشكل متتالٍ (الافتراضي 3 فتاوى متتالية)، يتوقف الفحص تلقائياً لتوفير موارد الشبكة ومفاتيح الـ API.
3. **الفحص الكامل الاختياري (Full Scan)**: خيار لفحص كافة الصفحات وتحديث قاعدة البيانات دون التوقف عند الفتاوى الموجودة مسبقاً.
4. **توليد المتجهات تلقائياً**: توليد متجه دلالي متوافق بطول 1024 بعداً باستخدام نموذج `@cf/baai/bge-m3` قبل الحفظ.

### كيفية التشغيل والاستخدام:

#### 1. التشغيل اليدوي (مرة واحدة):
لتشغيل الفحص التدريجي الذكي يدويًا من سطر الأوامر:
```bash
python manage.py scrape_fatwas
```

#### 2. تشغيل فحص كامل (Full Scan):
لفحص كافة الفتاوى والصفحات القديمة والجديدة وتعبئة قاعدة البيانات من الصفر:
```bash
python manage.py scrape_fatwas --full
```

#### 3. تشغيل الفاحص كخدمة خلفية دورية (Daemon):
لتشغيل الفاحص ليعمل بشكل دوري مستمر في الخلفية (على سبيل المثال، فحص كل 60 دقيقة):
```bash
python manage.py scrape_fatwas --daemon --interval 60
```

#### 4. التشغيل التلقائي مع خادم الويب (Django Background Thread):
يمكن جعل الفاحص يعمل تلقائياً في الخلفية داخل نفس عملية خادم Django عند بدء التشغيل دون الحاجة لأوامر إضافية. للقيام بذلك، أضف التكوين التالي إلى ملف `.env`:
```env
# تفعيل التشغيل التلقائي للفاحص
SCRAPER_AUTO_RUN=True
# الفاصل الزمني بالدقائق (مثال: 1440 دقيقة تعادل 24 ساعة)
